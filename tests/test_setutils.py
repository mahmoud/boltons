...
def big_popper():
    ...
    print()
    print(
        "popped {} items in {} seconds".format(
            start_size - end_size, end_time - start_time
        )
    )
...
def test_complement_set():
    ...
    )  # frozen complement can be a member of complement set
    assert (
        len({complement(frozenset()): 1, complement(frozenset()): 2}) == 1
    )  # hash works
    ...
    class OpOverloader:
        ...
        __rand__ = (
            ...
            __or__
        ) = __ror__ = __ior__ = __xor__ = __rxor__ = __sub__ = __isub__ = __and__

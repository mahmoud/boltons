...
def test_strip_ansi():
    assert strutils.strip_ansi("\x1b[0m\x1b[1;36mart\x1b[46;34m\xdc") == "art\xdc"
    assert strutils.strip_ansi("\x1b[0m\x1b[1;36mart\x1b[46;34m\xdc") == "artÜ"
    assert strutils.strip_ansi("╒══════╕\n│ \x1b[1mCell\x1b[0m │\n╘══════╛") == (
        "╒══════╕\n" "│ Cell │\n" "╘══════╛"
    )
    assert (
        strutils.strip_ansi("ls\r\n\x1B[00m\x1b[01;31mfile.zip\x1b[00m\r\n\x1b[01;31m")
        == "ls\r\nfile.zip\r\n"
    )
    assert (
        strutils.strip_ansi("\t\u001b[0;35mIP\u001b[0m\t\u001b[0;36m192.1.0.2\u001b[0m")
        == "\tIP\t192.1.0.2"
    )
...
def test_parse_int_list():
    ...
    assert strutils.parse_int_list("1,3,5-8,10-11,15,") == [
        1,
        3,
        5,
        6,
...
def test_format_int_list():
    assert (
        strutils.format_int_list([1, 3, 5, 6, 7, 8, 10, 11, 15]) == "1,3,5-8,10-11,15"
    )
    assert strutils.format_int_list([5, 6, 7, 8]) == "5-8"

    assert (
        strutils.format_int_list([1, 3, 5, 6, 7, 8, 10, 11, 15], delim_space=True)
        == "1, 3, 5-8, 10-11, 15"
    )
...
class TestMultiReplace(TestCase):
    ...
    def test_sub_with_regex(self):
        ...
        m = strutils.MultiReplace(
            {r"cat": "kedi", r"purple": "mor", r"q\w+?t": "dinglehopper"}, regex=True
        )
        self.assertEqual(

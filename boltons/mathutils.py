...
def ceil_from_iter(src, x, allow_equal=True, sorted_src=False):
    """Return the ceiling of *x*, the smallest integer or float from
    *src* that is greater than [or equal to] *x*.

    Args:
        src (iterable):     Iterable of arbitrary numbers (ints or floats).
        x (int or float):   Number to be tested.
        allow_equal (bool): Defaults to ``True``. Allows equality to
            be ignored if set to ``False``.
        sorted_src (bool): Defaults to ``False``. Allows potential
           performance increase for large *src* iterable if set to
           ``False``, as long as *src* is pre-sorted.
...
def floor_from_iter(src, x, allow_equal=True, sorted_src=False):
    """Return the floor of *x*, the largest integer or float from *src*
